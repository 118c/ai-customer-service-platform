...
Agent 路由结合领域匹配、历史成功率、延迟和监控惩罚。专业 Agent 连续两个采集周期达到异常阈值后进入隔离，恢复窗口结束后半开探测；不可用期间回退综合服务 Agent。主备模型均不可用时转入本地连续性通道。
...
- 模型、Agent、检索、记忆和业务连接器异常统一记录为降级事件，并向员工返回可操作的连续性提示。
- Redis 不可用时进程内检查点可维持单实例服务；该模式不支持跨实例恢复。
- Prometheus 按真实请求记录请求量与延迟分布，并暴露 Agent 成功率、平均延迟和隔离状态。
- 完整工作流评测报告集中写入 SQLite，质量控制台展示版本、通过率、检索指标、业务确认完整度与延迟趋势。
